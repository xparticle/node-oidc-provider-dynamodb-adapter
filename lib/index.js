...
class DynamoDBAdapter {
  ...
  async findByUid(uid) {
    ...
    const attrs = await this._getSession(this._getKeyId(uid))
  ...
  async _get(keyId) {
    console.log("Going to search with", keyId)
    return new Promise((resolve, reject) => {
  ...
  async _getSession(sessionId) {
    console.log("Going to search with", sessionId)
    return new Promise((resolve, reject) => {
      this.model.scan()
      .filterExpression('#uid = :uid')
      .expressionAttributeValues({ ':uid' : sessionId})
      .expressionAttributeNames({ '#uid' : 'payload.uid'})
      .exec((error, item) => {
        console.log("Search result", error, item)
        if (error) {
          reject(error)
